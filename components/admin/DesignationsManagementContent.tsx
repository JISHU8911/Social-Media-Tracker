...
export function DesignationsManagementContent({ hideNavbar = false }: { hideNavbar?: boolean }) {
  ...
  const handleAddDesignation = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/designations', {
        ...
        body: JSON.stringify({ designationName: newDesignationName.trim() }),
      });
  ...
  const handleSaveEdit = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/designations/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ designationName: editingName.trim() }),
      });
